...
parser.add_argument('-k', '--kvals', default='35,25', help='salt ratio k values, default 35,25')
parser.add_argument('--rc', default=0.0, type=float, help='cut off in um for regularisation, default 0.0 um')
...
fig, ax = plt.subplots(2, 2, figsize=(6, 6.2), sharex=True, sharey=True, dpi=args.dpi)

for i, k in enumerate(kvals):
    
    for j, Q in enumerate(Qvals):

        pore.update(Q=Q, k=k, rc=args.rc)
        ...
        ax[i, j].streamplot(z, x, uz, ux, linewidth=1.5, arrowsize=2, density=1.2) # drift streamlines
        ...
        if pore.fixed_points is not None:
            ...
            ax[i, j].scatter(z1, 0, s=80, color='tab:red', lw=4, marker='o', zorder=9)
            ax[i, j].scatter(z2, 0, s=120, color='tab:orange', lw=3, marker='+', zorder=9)
            ax[i, j].plot(z_sep, x_sep, lw=3, label='sep', color='tab:red', zorder=9, ls='dashed')

        ax[i, j].plot([0, 0], [pore.R1, w], lw=6, c='k') # represent pore ..
        ax[i, j].plot([0, 0], [-pore.R1, -w], lw=6, c='k') # .. other side

        ax[i, j].set_xlim(0, 2*w)
        ax[i, j].set_ylim(-w, w)
        ax[i, j].set_aspect('equal')

        ax[i, j].tick_params(direction='in', width=gen_lw, length=5, top=True, right=True, labelsize=tick_fs)

        for spine in ax[i, j].spines:
            ax[i, j].spines[spine].set_linewidth(gen_lw)

for ij in [0, 1]:
    ax[1, ij].set_xlabel(r'$z$ / µm', fontsize=label_fs)
    ax[1, ij].set_xticks(xticks)
    ax[ij, 0].set_yticks(yticks)
    ax[ij, 0].set_ylabel(r'$x$ / µm', fontsize=label_fs, labelpad=0)

bbox = dict(boxstyle='round', fc='w', ls='') #lw=gen_lw)

for ij, label in enumerate(['(a)', '(b)', '(c)', '(d)']):
    ax[ij//2, ij%2].annotate(label, (6.45, 3.1), fontsize=label_fs, bbox=bbox, zorder=10)
