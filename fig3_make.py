...
ΔS = np.array([action(max(z1, rc), Q) - action(z2, Q) for z1, z2, Q in zip(z1, z2, Q)])
...
for ax, label, ypos in (ax1, 'a', 3e3), (ax2, 'b', 1e3):
    ax.annotate(f'({label})', (2e-4, ypos), fontsize=label_fs)
